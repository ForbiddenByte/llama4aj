...
void lm_ggml_cuda_op_gated_linear_attn(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    LM_GGML_ASSERT(dst->src[4]->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(C % H == 0);
    LM_GGML_ASSERT(C / H == 64 || C / H == 128);
