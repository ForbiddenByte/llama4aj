...
void lm_ggml_cuda_op_pad(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const size_t s00 = nb00 / lm_ggml_type_size(src0->type);
    const size_t s01 = nb01 / lm_ggml_type_size(src0->type);
    const size_t s02 = nb02 / lm_ggml_type_size(src0->type);
    const size_t s03 = nb03 / lm_ggml_type_size(src0->type);
