...
void lm_ggml_cuda_conv_2d_transpose_p0(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    LM_GGML_ASSERT(kernel->type == LM_GGML_TYPE_F16 && input->type == LM_GGML_TYPE_F32 && dst->type == LM_GGML_TYPE_F32);
    ...
    LM_GGML_ASSERT(channels_in == kernel->ne[3]);
    LM_GGML_ASSERT(stride > 0);

    cudaStream_t st = ctx.stream();

    LM_GGML_ASSERT(lm_ggml_is_contiguous(input));
