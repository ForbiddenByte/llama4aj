...
#else // LM_GGML_USE_MUSA
namespace wmma = nvcuda::wmma;
#endif // LM_GGML_USE_MUSA
#elif defined(LM_GGML_USE_HIP)
...
__launch_bounds__(nwarps*lm_ggml_cuda_get_physical_warp_size(), 1)
...
#if defined(FLASH_ATTN_AVAILABLE) && (defined(LM_GGML_HIP_ROCWMMA_FATTN) && defined(LM_GGML_USE_WMMA_FATTN))
...
#pragma unroll
                for (int k0 = 0; k0 < FATTN_KQ_STRIDE/2; k0 += warp_size) {
                    ...
                    KQ_max_new = lm_ggml_cuda_hmax2(KQ_max_new, KQ2_tmp[k0/warp_size]);
                }
                KQ_max_new = __half2half2(warp_reduce_max<warp_size>(lm_ggml_cuda_hmax(__low2half(KQ_max_new), __high2half(KQ_max_new))));
...
#pragma unroll
    ...
    LM_GGML_UNUSED_VARS(Q, K, V, mask, sinks, KV_max, dst, dst_meta, scale,
        ...
        ne10, ne11, ne12, ne13,
              ...
              nb31, nb32, nb33);
    NO_DEVICE_CODE;
#endif // defined(FLASH_ATTN_AVAILABLE) && (defined(LM_GGML_HIP_ROCWMMA_FATTN) && defined(LM_GGML_USE_WMMA_FATTN))
}
...
void lm_ggml_cuda_flash_attn_ext_wmma_f16_case(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    const lm_ggml_tensor * KQV = dst;

    constexpr int nwarps = 4;

    constexpr int frag_m = cols_per_block == 8 && D % 32 == 0 ? 32 : 16;
    const int warp_size = lm_ggml_cuda_info().devices[lm_ggml_cuda_get_device()].warp_size;
...
void lm_ggml_cuda_flash_attn_ext_wmma_f16(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const enum lm_ggml_prec prec = lm_ggml_flash_attn_ext_get_prec(KQV);
    ...
    if (prec != LM_GGML_PREC_DEFAULT) {
        if (Q->ne[1] <= 32 || Q->ne[0] > 128) {
            ...
            switch (Q->ne[0]) {
                case 64:
                    lm_ggml_cuda_flash_attn_ext_wmma_f16_case< 64, cols_per_block, float>(ctx, dst);
                    break;
                case 80:
                    lm_ggml_cuda_flash_attn_ext_wmma_f16_case< 80, cols_per_block, float>(ctx, dst);
                    break;
                case 96:
                    lm_ggml_cuda_flash_attn_ext_wmma_f16_case< 96, cols_per_block, float>(ctx, dst);
                    break;
                case 112:
                    lm_ggml_cuda_flash_attn_ext_wmma_f16_case<112, cols_per_block, float>(ctx, dst);
                    break;
                case 128:
                    lm_ggml_cuda_flash_attn_ext_wmma_f16_case<128, cols_per_block, float>(ctx, dst);
                    break;
                case 256:
                    lm_ggml_cuda_flash_attn_ext_wmma_f16_case<256, cols_per_block, float>(ctx, dst);
                    break;
                default:
    ...
    if (Q->ne[1] <= 8 && Q->ne[0] % warp_size == 0) {
        ...
        switch (Q->ne[0]) {
            case 64:
                lm_ggml_cuda_flash_attn_ext_wmma_f16_case< 64, cols_per_block, half>(ctx, dst);
                break;
            case 96:
                lm_ggml_cuda_flash_attn_ext_wmma_f16_case< 96, cols_per_block, half>(ctx, dst);
                break;
            case 128:
                lm_ggml_cuda_flash_attn_ext_wmma_f16_case<128, cols_per_block, half>(ctx, dst);
                break;
            case 256:
                lm_ggml_cuda_flash_attn_ext_wmma_f16_case<256, cols_per_block, half>(ctx, dst);
                break;
            default:
                LM_GGML_ABORT("fatal error");
                break;
    ...
    if (Q->ne[1] <= 32) {
        ...
        switch (Q->ne[0]) {
            case 64:
                ...
                break;
            case 80:
                lm_ggml_cuda_flash_attn_ext_wmma_f16_case< 80, cols_per_block, half>(ctx, dst);
                break;
            case 96:
                lm_ggml_cuda_flash_attn_ext_wmma_f16_case< 96, cols_per_block, half>(ctx, dst);
                break;
            case 112:
                lm_ggml_cuda_flash_attn_ext_wmma_f16_case<112, cols_per_block, half>(ctx, dst);
                break;
            case 128:
                lm_ggml_cuda_flash_attn_ext_wmma_f16_case<128, cols_per_block, half>(ctx, dst);
                break;
