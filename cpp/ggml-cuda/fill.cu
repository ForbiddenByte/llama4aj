...
void lm_ggml_cuda_op_fill(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    void * dst_d = dst->data;
    ...
    const int64_t k = lm_ggml_nelements(dst);
    ...
    switch (dst->type) {
        ...
        case LM_GGML_TYPE_F16:
            fill_kernel<<<num_blocks, CUDA_FILL_BLOCK_SIZE, 0, stream>>>((half *)dst_d, k, lm_ggml_cuda_cast<half>(value));
            break;
        default:
