...
static void launch_bin_bcast_pack(const lm_ggml_tensor * src0, const lm_ggml_tensor * src1, lm_ggml_tensor * dst,
...

    {
        ...
        //int64_t ne00 = cne0[0]; LM_GGML_UNUSED(ne00);
        //int64_t ne01 = cne0[1]; LM_GGML_UNUSED(ne01);
        //int64_t ne02 = cne0[2]; LM_GGML_UNUSED(ne02);
        //int64_t ne03 = cne0[3]; LM_GGML_UNUSED(ne03);
        ...
        LM_GGML_ASSERT(nb0 % sizeof(dst_t) == 0);
        LM_GGML_ASSERT(nb1 % sizeof(dst_t) == 0);
        LM_GGML_ASSERT(nb2 % sizeof(dst_t) == 0);
        LM_GGML_ASSERT(nb3 % sizeof(dst_t) == 0);

        LM_GGML_ASSERT(nb00 % sizeof(src0_t) == 0);
        LM_GGML_ASSERT(nb01 % sizeof(src0_t) == 0);
        LM_GGML_ASSERT(nb02 % sizeof(src0_t) == 0);
        LM_GGML_ASSERT(nb03 % sizeof(src0_t) == 0);

        LM_GGML_ASSERT(nb10 % sizeof(src1_t) == 0);
        LM_GGML_ASSERT(nb11 % sizeof(src1_t) == 0);
        LM_GGML_ASSERT(nb12 % sizeof(src1_t) == 0);
        LM_GGML_ASSERT(nb13 % sizeof(src1_t) == 0);

        LM_GGML_ASSERT(s0 == 1);
        LM_GGML_ASSERT(s00 == 1);
        LM_GGML_ASSERT(s10 == 1);
...
struct bin_bcast_cuda {
    ...
    void operator()(const struct lm_ggml_tensor * src0, const struct lm_ggml_tensor * src1, struct lm_ggml_tensor * dst,
            const src0_t * src0_dd, const src1_t * src1_dd, dst_t * dst_dd,
            cudaStream_t stream) {
...
template<class op>
static void lm_ggml_cuda_op_bin_bcast(
    const lm_ggml_tensor * src0, const lm_ggml_tensor * src1, lm_ggml_tensor * dst,
    const void * src0_dd, const void * src1_dd, void * dst_dd, cudaStream_t stream) {

    LM_GGML_ASSERT(src1->type == LM_GGML_TYPE_F32 || src1->type == LM_GGML_TYPE_F16);

    if (src0->type == LM_GGML_TYPE_F32 && dst->type == LM_GGML_TYPE_F32) {
        op()(src0, src1, dst, (const float *)src0_dd, (const float *)src1_dd, (float *)dst_dd, stream);
    } else if (src0->type == LM_GGML_TYPE_F16 && src1->type == LM_GGML_TYPE_F16 && dst->type == LM_GGML_TYPE_F16) {
...
void lm_ggml_cuda_op_repeat(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    lm_ggml_cuda_op_bin_bcast<bin_bcast_cuda<op_repeat, 0>>(dst, dst->src[0], dst, nullptr, dst->src[0]->data, dst->data, ctx.stream());
}

void lm_ggml_cuda_op_add(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    lm_ggml_cuda_op_bin_bcast<bin_bcast_cuda<op_add>>(dst->src[0], dst->src[1], dst, dst->src[0]->data, dst->src[1]->data, dst->data, ctx.stream());
}

void lm_ggml_cuda_op_sub(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    lm_ggml_cuda_op_bin_bcast<bin_bcast_cuda<op_sub>>(dst->src[0], dst->src[1], dst, dst->src[0]->data, dst->src[1]->data, dst->data, ctx.stream());
}

void lm_ggml_cuda_op_mul(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    lm_ggml_cuda_op_bin_bcast<bin_bcast_cuda<op_mul>>(dst->src[0], dst->src[1], dst, dst->src[0]->data, dst->src[1]->data, dst->data, ctx.stream());
}

void lm_ggml_cuda_op_div(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    lm_ggml_cuda_op_bin_bcast<bin_bcast_cuda<op_div>>(dst->src[0], dst->src[1], dst, dst->src[0]->data, dst->src[1]->data, dst->data, ctx.stream());
}

template <float (*op)(const float, const float), int n_fuse>
static void lm_ggml_cuda_op_fused_binbcast_impl(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    if (src0->type == LM_GGML_TYPE_F32 && dst->type == LM_GGML_TYPE_F32) {
        ...
        fprintf(stderr,
                ...
                __func__, lm_ggml_type_name(dst->type), lm_ggml_type_name(src0->type), lm_ggml_type_name(src1->type));
        LM_GGML_ABORT("fatal error");
    }
}
...
void lm_ggml_cuda_op_fused_add(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst, int n_fuse) {
    LM_GGML_ASSERT(2 <= n_fuse && n_fuse <= 8);

    switch (n_fuse) {
        case 2:
            lm_ggml_cuda_op_fused_binbcast_impl<op_add, 2>(ctx, dst);
            break;
        case 3:
            lm_ggml_cuda_op_fused_binbcast_impl<op_add, 3>(ctx, dst);
            break;
        case 4:
            lm_ggml_cuda_op_fused_binbcast_impl<op_add, 4>(ctx, dst);
            break;
        case 5:
            lm_ggml_cuda_op_fused_binbcast_impl<op_add, 5>(ctx, dst);
            break;
        case 6:
            lm_ggml_cuda_op_fused_binbcast_impl<op_add, 6>(ctx, dst);
            break;
        case 7:
            lm_ggml_cuda_op_fused_binbcast_impl<op_add, 7>(ctx, dst);
            break;
        case 8:
            lm_ggml_cuda_op_fused_binbcast_impl<op_add, 8>(ctx, dst);
            break;
        default:
            LM_GGML_ASSERT(false && "Unsupported n_fuse value");
    }
}
...
void lm_ggml_cuda_op_repeat_back(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    LM_GGML_ASSERT(ne2*ne3 <= (1 << 15));

    const size_t ts = lm_ggml_type_size(src0->type);
