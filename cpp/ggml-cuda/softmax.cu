...
static void soft_max_f32_cuda(const float *                                x,
                              ...
                              cudaStream_t                                 stream,
                              [[maybe_unused]] lm_ggml_backend_cuda_context & ctx) {
    int nth = WARP_SIZE;
    ...
    const size_t nbytes_shared = (LM_GGML_PAD(ncols_x, WARP_SIZE) + WARP_SIZE)*sizeof(float);
    ...
    if (nbytes_shared <= smpbo) {
        ...
        if (lm_ggml_cuda_info().devices[id].supports_cooperative_launch &&
            ncols_x / (params.ne01 * params.ne02 * params.ne03) > 8192 && mask == nullptr && sinks == nullptr &&
            params.scale == 1.0f && params.max_bias == 0.0f) {
            lm_ggml_cuda_pool_alloc<float> tmp_maxs_alloc(ctx.pool(), lm_ggml_cuda_info().devices[id].nsm * sizeof(float));
            lm_ggml_cuda_pool_alloc<float> tmp_sums_alloc(ctx.pool(), lm_ggml_cuda_info().devices[id].nsm * sizeof(float));
            ...
            CUDA_CHECK(cudaLaunchCooperativeKernel((void *) soft_max_f32_parallelize_cols,
                                                   dim3(lm_ggml_cuda_info().devices[id].nsm, 1, 1),
...
void lm_ggml_cuda_op_soft_max(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    LM_GGML_ASSERT(!src1 || src1->type == LM_GGML_TYPE_F16 || src1->type == LM_GGML_TYPE_F32); // src1 contains mask and it is optional
...
void lm_ggml_cuda_op_soft_max_back(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    const lm_ggml_tensor * src0 = dst->src[0]; // grad
    const lm_ggml_tensor * src1 = dst->src[1]; // forward pass output
