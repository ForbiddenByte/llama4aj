...
static void ssm_scan_f32_cuda(const float * src0, const float * src1, const float * src2, const float * src3,
    ...
    if (src3_nb1 == sizeof(float)) {
        ...
        if (d_state == 128) {
            ...
            LM_GGML_ABORT("doesn't support d_state!=(128 or 256).");
        }
    } else {
        // Mamba-1
        constexpr int threads = 128;
        LM_GGML_ASSERT(n_head % threads == 0);
        LM_GGML_ASSERT(head_dim == 1);
        LM_GGML_ASSERT(n_group == 1);
        ...
        if (d_state == 16) {
            ...
            LM_GGML_ABORT("doesn't support d_state!=16.");
        }
    }
}
...
void lm_ggml_cuda_op_ssm_scan(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const struct lm_ggml_tensor * src6 = dst->src[6];  // ids
    ...
    LM_GGML_ASSERT(lm_ggml_nelements(src1) + nc*nr*nh*n_s == lm_ggml_nelements(dst));
    ...
    LM_GGML_ASSERT(src6->type == LM_GGML_TYPE_I32);
