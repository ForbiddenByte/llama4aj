...
void lm_ggml_cuda_opt_step_adamw(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    LM_GGML_ASSERT(src0_grad_m->type  == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(src0_grad_v->type  == LM_GGML_TYPE_F32);
    ...
    LM_GGML_ASSERT(lm_ggml_is_contiguous(src0_grad_m));
    LM_GGML_ASSERT(lm_ggml_is_contiguous(src0_grad_v));
    LM_GGML_ASSERT(lm_ggml_is_contiguous(adamw_params));
