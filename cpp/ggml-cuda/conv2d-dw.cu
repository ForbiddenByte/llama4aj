...
void lm_ggml_cuda_op_conv2d_dw(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const lm_ggml_tensor * input  = dst->src[1];

    LM_GGML_ASSERT(kernel->type == LM_GGML_TYPE_F32 && input->type == LM_GGML_TYPE_F32 && dst->type == LM_GGML_TYPE_F32);
    ...
    if (lm_ggml_is_contiguous(input)) {
    ...
    } else if (lm_ggml_is_contiguous_channels(input)) {
        conv2d_dw_kernel<float, cwhn_layout><<<blocks, CUDA_CONV2D_DW_BLOCK_SIZE, 0, st>>>(
            x_d, w_d, y_d, in_w, in_h, out_w, out_h, kernel_w, kernel_h, stride_x, stride_y, padding_x, padding_y,
            dilation_x, dilation_y, channels, batches);
    } else {
        LM_GGML_ABORT("Unsupported memory layout for conv_2d_dw");
    }
}
