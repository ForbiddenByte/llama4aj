...
void lm_ggml_cuda_count_equal(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const int nsm = lm_ggml_cuda_info().devices[lm_ggml_cuda_get_device()].nsm;

    const int64_t ne = lm_ggml_nelements(src0);
    LM_GGML_ASSERT(ne < (1 << 30) && "atomicAdd implementation only supports int");
    const int64_t dne = LM_GGML_PAD((ne + 4*nsm - 1) / (4*nsm), CUDA_COUNT_EQUAL_CHUNK_SIZE);

    CUDA_CHECK(cudaMemsetAsync(dst_d, 0, lm_ggml_nbytes(dst), stream));
