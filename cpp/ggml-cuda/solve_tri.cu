...
static void solve_tri_f32_cublas(lm_ggml_backend_cuda_context & ctx,
                                 const float *               A,
                                 const float *               B,
                                 float *                     X,
                                 int                         n,
    ...
    lm_ggml_cuda_pool_alloc<const float *> A_ptrs_alloc(ctx.pool(id), total_batches);
    lm_ggml_cuda_pool_alloc<float *>       X_ptrs_alloc(ctx.pool(id), total_batches);
...
void lm_ggml_cuda_op_solve_tri(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    const lm_ggml_tensor * src0 = dst->src[0];  // A (n×n, lower triangular)
    const lm_ggml_tensor * src1 = dst->src[1];  // B (n×k)
