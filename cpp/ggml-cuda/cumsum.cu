...
#pragma unroll
        ...
        float val = (idx < ne00) ? lm_ggml_cuda_cast<float, T>(temp[num_unroll - 1]) : 0.0f;
...
#pragma unroll
        for (int32_t j = 0; j < num_unroll; j++) {
            if (idx + j < ne00) {
                dst_row[idx + j] = temp[j] + lm_ggml_cuda_cast<T, float>(final_val_offset);
            }
        }
...
#ifdef LM_GGML_CUDA_USE_CUB
template <typename T>
static void cumsum_cub(lm_ggml_cuda_pool & pool,
                       const T *        src,
                       T *              dst,
                       int64_t          ne,
                       cudaStream_t     stream) {
...
static void cumsum_cuda(
        [[maybe_unused]] lm_ggml_backend_cuda_context & ctx, const T * src, T * dst,
        const int64_t ne00, const int64_t ne01, const int64_t ne02, const int64_t ne03,
    ...
    const auto &info = lm_ggml_cuda_info().devices[lm_ggml_cuda_get_device()];
