...
void argsort_f32_i32_cuda_cub(lm_ggml_cuda_pool & pool,
                              ...
                              cudaStream_t     stream) {
    lm_ggml_cuda_pool_alloc<int>   temp_indices_alloc(pool, ncols * nrows);
    lm_ggml_cuda_pool_alloc<float> temp_keys_alloc(pool, ncols * nrows);
...
#else
    lm_ggml_cuda_pool_alloc<int> offsets_alloc(pool, nrows + 1);
    ...
    lm_ggml_cuda_pool_alloc<uint8_t> temp_storage_alloc(pool, temp_storage_bytes);
...
template<typename T>
static inline __device__ void lm_ggml_cuda_swap(T & a, T & b) {
    T tmp = a;
    a = b;
    b = tmp;
}
            ...
            if (ixj > col) {
                if ((col & k) == 0) {
                    if (dst_row[col] >= ncols ||
                        ...
                        lm_ggml_cuda_swap(dst_row[col], dst_row[ixj]);
                    }
                } else {
...
void argsort_f32_i32_cuda_bitonic(const float *   x,
    ...
    LM_GGML_ASSERT(shared_mem <= lm_ggml_cuda_info().devices[lm_ggml_cuda_get_device()].smpb);

    if (order == LM_GGML_SORT_ORDER_ASC) {
        k_argsort_f32_i32<LM_GGML_SORT_ORDER_ASC>
            <<<block_nums, block_dims, shared_mem, stream>>>(x, dst, ncols, ncols_pad);
    } else if (order == LM_GGML_SORT_ORDER_DESC) {
        k_argsort_f32_i32<LM_GGML_SORT_ORDER_DESC>
            <<<block_nums, block_dims, shared_mem, stream>>>(x, dst, ncols, ncols_pad);
    } else {
...
void lm_ggml_cuda_op_argsort(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const int    ncols_pad      = next_power_of_2(ncols);
    const size_t shared_mem     = ncols_pad * sizeof(int);
    const size_t max_shared_mem = lm_ggml_cuda_info().devices[lm_ggml_cuda_get_device()].smpb;
