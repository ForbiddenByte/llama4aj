...
static  __global__ void pool2d_nchw_kernel(
        ...
        const Ti* src, To* dst, const enum lm_ggml_op_pool op) {
...
#endif
            switch (op) {
                case LM_GGML_OP_POOL_AVG: res += cur * scale; break;
                case LM_GGML_OP_POOL_MAX: res = max(res, (To)cur); break;
                default: assert(false);
            }
        }
    }
...
static void pool2d_nchw_kernel_f32_f32_cuda(
        ...
        const int kh, const int kw, const int sh, const int sw,
        const int ph, const int pw, const int parallel_elements,
        const float * src, float * dst, const enum lm_ggml_op_pool op,
        cudaStream_t stream) {
...
void lm_ggml_cuda_op_pool2d(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    enum lm_ggml_op_pool op = static_cast<lm_ggml_op_pool>(opts[0]);
    const int k0 = opts[1];
    const int k1 = opts[2];
    const int s0 = opts[3];
    const int s1 = opts[4];
