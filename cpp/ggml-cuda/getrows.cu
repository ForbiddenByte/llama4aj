...
static __global__ void k_get_rows(
    ...
    for (int64_t z = blockIdx.z; z < ne11*ne12; z += gridDim.z) {
        for (int64_t i00 = 2*(blockIdx.y*blockDim.x + threadIdx.x); i00 < ne00; i00 += gridDim.y*blockDim.x) {
            ...
            dst_row[iybs + iqs + 0]        = lm_ggml_cuda_cast<dst_t>(v.x);
            dst_row[iybs + iqs + y_offset] = lm_ggml_cuda_cast<dst_t>(v.y);
        }
    }
}
...
static __global__ void k_get_rows_float(
    ...
    for (int64_t z = blockIdx.z; z < ne11*ne12; z += gridDim.z) {
        for (int64_t i00 = blockIdx.y*blockDim.x + threadIdx.x; i00 < ne00; i00 += gridDim.y*blockDim.x) {
            ...
            dst_row[i00] = lm_ggml_cuda_cast<dst_t>(src0_row[i00]);
        }
    }
}
...
static void lm_ggml_cuda_get_rows_switch_src0_type(
        const void * src0_d, const lm_ggml_type src0_type, const int32_t * src1_d, dst_t * dst_d,
        const int64_t ne00, const size_t nb01, const size_t nb02, const size_t nb03,
        const int64_t ne10, const int64_t ne11, const int64_t ne12, const size_t nb10, const size_t nb11, const size_t nb12,
        const size_t nb1, const size_t nb2, const size_t nb3,
        cudaStream_t stream) {
    switch (src0_type) {
        ...
        default:
            // TODO: k-quants
            LM_GGML_ABORT("%s: unsupported src0 type: %s\n", __func__, lm_ggml_type_name(src0_type));
            break;
    }
}
...
void get_rows_cuda(
    ...
    switch (dst_type) {
        case LM_GGML_TYPE_F32:
            lm_ggml_cuda_get_rows_switch_src0_type(src0_d, src0_type, src1_d, (float *) dst_d,
                ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
            break;
        case LM_GGML_TYPE_I32:
            lm_ggml_cuda_get_rows_switch_src0_type(src0_d, src0_type, src1_d, (int32_t *) dst_d,
                ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
            break;
        case LM_GGML_TYPE_F16:
            lm_ggml_cuda_get_rows_switch_src0_type(src0_d, src0_type, src1_d, (half *) dst_d,
                ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
            break;
        case LM_GGML_TYPE_BF16:
            lm_ggml_cuda_get_rows_switch_src0_type(src0_d, src0_type, src1_d, (nv_bfloat16 *) dst_d,
                ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
            break;
        default:
            LM_GGML_ABORT("%s: unsupported dst type: %s\n", __func__, lm_ggml_type_name(dst_type));
            break;
    }
}
...
void lm_ggml_cuda_op_get_rows(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    LM_GGML_ASSERT(dst->nb[0]  == lm_ggml_type_size(dst->type));
...
void lm_ggml_cuda_op_get_rows_back(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const lm_ggml_tensor * src1 = dst->src[1]; // src1 in forward pass
    ...
    LM_GGML_ASSERT(ne02*ne03 == 1);
    LM_GGML_ASSERT(ne12*ne13 == 1);
    LM_GGML_ASSERT(ne2*ne3 == 1);
