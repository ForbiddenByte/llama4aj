...
static __global__ void tri_kernel(
    ...
    if constexpr (prefix_keep) {
        ...
        for (int64_t i0 = threadIdx.x + split_point; i0 < ne00; i0 += blockDim.x) {
            dst_row[i0] = lm_ggml_cuda_cast<T, float>(0.0f);
        }
    } else {
        for (int64_t i0 = threadIdx.x; i0 < split_point; i0 += blockDim.x) {
            dst_row[i0] = lm_ggml_cuda_cast<T, float>(0.0f);
...
static void tri_cuda(
    ...
    const int add_to_split = (ttype == LM_GGML_TRI_TYPE_LOWER_DIAG || ttype == LM_GGML_TRI_TYPE_UPPER) ? 1 : 0;
    const bool prefix_keep = (ttype == LM_GGML_TRI_TYPE_LOWER || ttype == LM_GGML_TRI_TYPE_LOWER_DIAG);
...
void lm_ggml_cuda_op_tri(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const lm_ggml_tri_type ttype = static_cast<lm_ggml_tri_type>(lm_ggml_get_op_params_i32(dst, 0));
