...
static __global__ void k_set_rows(const src_t * __restrict__ src0,
    ...
    dst_row_ptr[i00] = lm_ggml_cuda_cast<dst_t>(src0_row[i00]);
...
static void set_rows_cuda(lm_ggml_backend_cuda_context & ctx, const lm_ggml_tensor * src0, const lm_ggml_tensor * src1, lm_ggml_tensor * dst) {
    ...
    if (dst->type == LM_GGML_TYPE_F32) {
    ...
    } else if (dst->type == LM_GGML_TYPE_Q4_0) {
    ...
    } else if (dst->type == LM_GGML_TYPE_Q4_1) {
    ...
    } else if (dst->type == LM_GGML_TYPE_Q5_0) {
    ...
    } else if (dst->type == LM_GGML_TYPE_Q5_1) {
    ...
    } else if (dst->type == LM_GGML_TYPE_Q8_0) {
    ...
    } else if (dst->type == LM_GGML_TYPE_IQ4_NL) {
        ...
        LM_GGML_ABORT("unsupported type %s", lm_ggml_type_name(dst->type));
    }
}
