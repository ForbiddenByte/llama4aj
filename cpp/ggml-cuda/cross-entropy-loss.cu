...
void lm_ggml_cuda_cross_entropy_loss(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    lm_ggml_cuda_pool_alloc<float> dst_tmp(pool, blocks_num.x);
...
void lm_ggml_cuda_cross_entropy_loss_back(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const lm_ggml_tensor * src0f = dst->src[1];
    const lm_ggml_tensor * src1f = dst->src[2];

    LM_GGML_ASSERT(src0f->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(src1f->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT( grad->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(  dst->type == LM_GGML_TYPE_F32);

    LM_GGML_ASSERT(lm_ggml_is_scalar(grad));
    LM_GGML_ASSERT(lm_ggml_is_contiguous(src0f));
    LM_GGML_ASSERT(lm_ggml_is_contiguous(src1f));
    LM_GGML_ASSERT(lm_ggml_is_contiguous(dst));
    LM_GGML_ASSERT(lm_ggml_are_same_shape(src0f, src1f));
    LM_GGML_ASSERT(lm_ggml_are_same_shape(src0f, dst));
