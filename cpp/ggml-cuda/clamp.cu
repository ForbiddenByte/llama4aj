...
void lm_ggml_cuda_op_clamp(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    if (src0->type == LM_GGML_TYPE_F16) {
        clamp_cuda((const half *)src0_d, (half *)dst_d, (half)min, (half)max, lm_ggml_nelements(src0), stream);
    } else {
        clamp_cuda((const float *)src0_d, (float *)dst_d, (float)min, (float)max, lm_ggml_nelements(src0), stream);
    }
}
