...
void lm_ggml_cuda_op_norm(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const size_t ts0 = lm_ggml_type_size(src0->type);
    LM_GGML_ASSERT(nb00 == ts0);
...
void lm_ggml_cuda_op_group_norm(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    group_norm_f32_cuda(src0_d, dst_d, num_groups * src0->ne[3], eps, group_size, lm_ggml_nelements(src0), stream);
}
...
void lm_ggml_cuda_op_rms_norm_fused(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst, lm_ggml_tensor * mul_tensor) {
    const lm_ggml_tensor * rms_norm_src = (lm_ggml_tensor *) dst->src[0];
    float eps = 0.0f;
    ...
    const lm_ggml_tensor * mul_src = nullptr;
    ...
    LM_GGML_ASSERT(rms_norm_src->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(dst->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(mul_tensor->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(eps >= 0.0f);
    ...
    const size_t ts0 = lm_ggml_type_size(rms_norm_src->type);
    LM_GGML_ASSERT(rms_norm_src->nb[0] == ts0);
    const int64_t s01 = rms_norm_src->nb[1] / ts0;
    const int64_t s02 = rms_norm_src->nb[2] / ts0;
    const int64_t s03 = rms_norm_src->nb[3] / ts0;

    const size_t ts_mul = lm_ggml_type_size(mul_src->type);
    LM_GGML_ASSERT(mul_src->nb[0] == ts_mul);
...
void lm_ggml_cuda_op_rms_norm_fused_add(lm_ggml_backend_cuda_context & ctx,
    ...
    const lm_ggml_tensor * add_src = nullptr;
    ...
    LM_GGML_ASSERT(dst->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(mul_tensor->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(add_tensor->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(eps >= 0.0f);
    ...
    const size_t ts_add = lm_ggml_type_size(add_src->type);
    LM_GGML_ASSERT(add_src->nb[0] == ts_add);
...
void lm_ggml_cuda_op_rms_norm_back(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    const lm_ggml_tensor * grad  = dst->src[0]; // gradients
    const lm_ggml_tensor * src0f = dst->src[1]; // src0 from forward pass
    ...
    LM_GGML_ASSERT( grad->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(src0f->type == LM_GGML_TYPE_F32);
    LM_GGML_ASSERT(  dst->type == LM_GGML_TYPE_F32);

    const int64_t ne00 = src0f->ne[0];
    const int64_t nrows = lm_ggml_nrows(src0f);
