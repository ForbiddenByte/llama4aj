...
#pragma unroll
        ...
        scales[b] = e;
        const float inv_s = (amax == 0.0f) ? 0.0f : __frcp_rn(lm_ggml_cuda_e8m0_to_fp32(e));
        ...
        const uint8_t q_val = lm_ggml_cuda_float_to_fp4_e2m1(xi, inv_s);
...
void quantize_row_q8_1_cuda(
        const float * x, const int32_t * ids, void * vy, const lm_ggml_type type_src0,
        const int64_t ne00, const int64_t s01, const int64_t s02, const int64_t s03,
        const int64_t ne0, const int64_t ne1, const int64_t ne2, const int64_t ne3, cudaStream_t stream) {
    LM_GGML_ASSERT(!ids);
    LM_GGML_ASSERT(ne0 % QK8_1 == 0);
...
void quantize_mmq_mxfp4_cuda(const float *                    x,
                             const int32_t *                  ids,
                             void *                           vy,
                             [[maybe_unused]] const lm_ggml_type type_src0,
                             const int64_t                    ne00,
                             const int64_t                    s01,
                             const int64_t                    s02,
                             const int64_t                    s03,
                             ...
                             const int64_t                    ne3,
                             cudaStream_t                     stream) {
    LM_GGML_ASSERT(ne0 % (2 * QK_MXFP4) == 0);
