...
static  __global__ void im2col_3d_kernel(
    ...
    LM_GGML_UNUSED(N); LM_GGML_UNUSED(OC); LM_GGML_UNUSED(OH_OW); LM_GGML_UNUSED(OD); LM_GGML_UNUSED(OW); LM_GGML_UNUSED(KD); LM_GGML_UNUSED(KH);
    LM_GGML_UNUSED(ID_IH_IW); LM_GGML_UNUSED(IH_IW); LM_GGML_UNUSED(IC_ID_IH_IW); LM_GGML_UNUSED(OW_KD_KH_KW);
...
void lm_ggml_cuda_op_im2col_3d(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    const size_t  es       = lm_ggml_element_size(src1);
