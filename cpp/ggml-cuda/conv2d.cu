    ...
    for (int64_t c_in = 0; c_in < P.IC; ++c_in) {
        ...
        for (int64_t ky = bounds.y_min; ky < bounds.y_max; ++ky) {
            ...
            for (int64_t kx = bounds.x_min; kx < bounds.x_max; ++kx) {
                ...
                acc += (input_val * lm_ggml_cuda_cast<float>(kernel_val));
            }
        }
    }
...
void lm_ggml_cuda_op_conv2d(lm_ggml_backend_cuda_context & ctx, lm_ggml_tensor * dst) {
    ...
    LM_GGML_ASSERT(kernel->type == LM_GGML_TYPE_F16 || kernel->type == LM_GGML_TYPE_F32);

    // same number of input channels
    LM_GGML_ASSERT(input->ne[2] == kernel->ne[2]);
    ...
    if (kernel->type == LM_GGML_TYPE_F16) {
