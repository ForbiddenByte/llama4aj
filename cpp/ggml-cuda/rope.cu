...
void lm_ggml_cuda_op_rope_impl(lm_ggml_backend_cuda_context & ctx,
    ...
    const size_t s03 = src0->nb[3] / lm_ggml_type_size(src0->type);

    const size_t s1 = dst->nb[1] / lm_ggml_type_size(dst->type);
    const size_t s2 = dst->nb[2] / lm_ggml_type_size(dst->type);
    const size_t s3 = dst->nb[3] / lm_ggml_type_size(dst->type);
